// Shared types, helpers, and Webflow API utilities used by multiple scripts.
...
export const getImageHash = (url: string | null) => imageHashesCache.getImageHash(url);
...
export async function sameImage(url1: string | null, url2: string | null): Promise<boolean> {
    const [h1, h2] = await Promise.all([getImageHash(url1), getImageHash(url2)]);
    return h1 === h2;
}

export async function sameImages(urls1: string[], urls2: string[]): Promise<boolean> {
...
export async function pickImage(url: string, base?: WebflowFileRef): Promise<string> {
...
export async function pickImages(urls: string[], base?: WebflowFileRef[]): Promise<string[]> {
...
export function isSameRefs(a: string[], b: string[]): boolean {
...
export interface IWebflowItemType {
    new(i: Webflow.CollectionItem): IWebflowItem
    generateFields(i: IWebsiteItem): Webflow.CollectionItemFieldData;
}

export function fieldsToItem(fields: Webflow.CollectionItemFieldData): Webflow.CollectionItem {
    return {
        isDraft: false,
        isArchived: false,
        fieldData: fields
...
export interface IWebflowItem extends IWebsiteItem {
    item: ExistingCollectionItem;
}

// ExistingCollectionItem narrows CollectionItem to always have an id, as
// returned by GET/PUT responses (the Webflow openapi spec incorrectly marks
// id as optional everywhere).
export type ExistingCollectionItem = Webflow.CollectionItem & { id: string };

// WebsiteMapTag is the internal representation of a map tag used in this script.
export interface WebsiteMapTag extends IWebsiteItem { }

export function isWebsiteMapTagEqual(a: WebsiteMapTag, b: WebsiteMapTag): boolean {
    return a.name === b.name && a.slug === b.slug;
}

export interface WebflowMapTag extends WebsiteMapTag { }

// WebflowMapTag is the native Webflow representation of a tag as used by the
// Webflow API.
export class WebflowMapTag implements IWebflowItem {
    item: ExistingCollectionItem;

    constructor(item: Webflow.CollectionItem) {
        this.item = item as ExistingCollectionItem;
...
export interface WebsiteMapTerrain extends IWebsiteItem { }

export function isWebsiteMapTerrainEqual(a: WebsiteMapTerrain, b: WebsiteMapTerrain): boolean {
    return a.name === b.name && a.slug === b.slug;
}

export interface WebflowMapTerrain extends WebsiteMapTerrain { }

// WebflowMapTerrain is the native Webflow representation of a Terrain as used by the
// Webflow API.
export class WebflowMapTerrain implements IWebflowItem {
    item: ExistingCollectionItem;
...
// webflow.
export interface WebsiteMapInfo {
    name: string;
    rowyId: string;
    minimapUrl: string;
...
export async function isWebflowMapInfoEqual(a: WebsiteMapInfo, b: WebsiteMapInfo): Promise<boolean> {
...
export interface WebflowMapInfo extends WebsiteMapInfo { }

// WebflowMap is the native Webflow representation of data as used by the
// Webflow API.
export class WebflowMapInfo {
    item: ExistingCollectionItem;
...
export async function buildWebflowInfo(
    maps: MapList,
...
export async function getFieldCollection(field: keyof WebflowMapFieldsRead, mapCollection: Webflow.Collection, webflow: WebflowClient): Promise<Webflow.Collection> {
...
export function resolveItemRefsInMapInfos(mapInfos: Map<string, WebsiteMapInfo>, field: 'mapTags' | 'mapTerrains', refs: Map<string, IWebflowItem>, dryRun: boolean) {
...
export async function getAllWebflowItems(collection: Webflow.Collection, webflow: WebflowClient, limiter: Bottleneck): Promise<Webflow.CollectionItem[]> {
...
export async function getAllWebflowMaps(mapsCollection: Webflow.Collection, webflow: WebflowClient, limiter: Bottleneck): Promise<Map<string, WebflowMapInfo>> {
    const items = await getAllWebflowItems(mapsCollection, webflow, limiter);
...
export async function getAllWebflowMapTags(mapTagsCollection: Webflow.Collection, webflow: WebflowClient, limiter: Bottleneck): Promise<Map<string, WebflowMapTag>> {
    const items = await getAllWebflowItems(mapTagsCollection, webflow, limiter);
    const tags = items.map(item => new WebflowMapTag(item));
    return new Map(tags.map(tag => [tag.slug, tag]));
}

// getAllWebflowMapTerrains returns all map terrains from the Webflow collection mapped by terrain slug.
export async function getAllWebflowMapTerrains(mapTerrainsCollection: Webflow.Collection, webflow: WebflowClient, limiter: Bottleneck): Promise<Map<string, WebflowMapTerrain>> {
    const items = await getAllWebflowItems(mapTerrainsCollection, webflow, limiter);
